...
public class OrderTransaction {
	...
	public void VerifyOrderTransactionData() throws IOException, InterruptedException, SQLException, ParseException {
		...
		{		
			...
			if (_CP.rs.next()) {
				try {
					Assert.assertEquals(OrderTransactionJsonData.get("admin_graphql_api_id").toString(),
							_CP.rs.getString("admin_graphql_api_id").toString(), "admin_graphql_api_id not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("amount").toString(),
							_CP.rs.getString("amount").toString(), "amount not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("authorization").toString(),
							_CP.rs.getString("authorization_").toString(),"authorization not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("currency").toString(),
							_CP.rs.getString("currency").toString(),"currency not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("device_id").toString(),
							_CP.rs.getString("device_id").toString(),"device_id not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("error_code").toString(),
							_CP.rs.getString("error_code").toString(),"error_code not Match in Row" + Iteration);
					
 				    Assert.assertEquals(OrderTransactionJsonData.get("gateway").toString(),
							_CP.rs.getString("gateway").toString(),"gateway not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("kind").toString(),
							_CP.rs.getString("kind").toString(),"kind not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("location_id").toString()
							,_CP.rs.getString("location_id").toString(),"location_id not Match in Row" + Iteration);
										
					Assert.assertEquals(OrderTransactionJsonData.get("message").toString(),
							_CP.rs.getString("message").toString(),"message not Match in Row" + Iteration);
										
					Assert.assertEquals(OrderTransactionJsonData.get("order_id").toString(),
							_CP.rs.getString("order_id").toString(),"order_id not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("parent_id").toString(),
							_CP.rs.getString("parent_id").toString(),"parent_id not Match in Row" + Iteration);
					...
					Assert.assertEquals(OrderTransactionJsonData.get("receipt").toString(),
						   _CP.rs.getString("receipt").toString(),"receipt not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("source_name").toString(),
							_CP.rs.getString("source_name").toString(),"source_name not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("status").toString(),
							_CP.rs.getString("status").toString(),"status not Match in Row" + Iteration);
					
					Assert.assertEquals(OrderTransactionJsonData.get("test").toString(),
